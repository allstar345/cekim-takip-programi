...
            if (error) { console.error('Ekip kaydedilirken hata oluştu:', error); Swal.fire('Hata!', 'Ekip planı kaydedilirken bir hata oluştu.', 'error'); } 
...
            if (error) { console.error('İzinliler kaydedilirken hata oluştu:', error); Swal.fire('Hata!', 'İzinli listesi kaydedilirken bir hata oluştu.', 'error'); } 
            else { renderCurrentPage(); }
        }
    }
...
            Swal.fire({ title: 'Emin misiniz?', text: "Bu çekim planı kalıcı olarak silinecektir!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Evet, sil!', cancelButtonText: 'İptal'}).then(async (result) => { if (result.isConfirmed) { await db.from('shoots').delete().eq('id', id); } });
        }
        if (buttonInTable.classList.contains('edit-btn')) {
            const id = buttonInTable.getAttribute('data-id');
            const shootToEdit = allShoots.find(shoot => shoot.id === Number(id)); 
            if (shootToEdit) { populateFormForEdit(shootToEdit); }
        }
    }
});
...
    const shootData = { studio: formData.get('studio'), teacher: formData.get('teacher'), date: formData.get('date'), day: formData.get('day'), start_time: formData.get('start_time'), end_time: formData.get('end_time'), director: formData.get('director'), kameraman_1: formData.get('kameraman_1'), kameraman_2: formData.get('kameraman_2'), shoot_code: formData.get('shoot_code'), content: formData.get('content') };
    if (shootData.start_time >= shootData.end_time) { Swal.fire('Hata!', 'Bitiş saati, başlangıç saatinden sonra olmalıdır.', 'error'); return; }
...
    if (onLeaveToday.includes(shootData.director)) { Swal.fire('Hata!', `Seçtiğiniz yönetmen (${shootData.director}) bu gün için izinli olarak işaretlenmiş. Lütfen farklı bir yönetmen seçin veya izin planını güncelleyin.`, 'error'); return; }
    if (currentEditId === null) {
        const { data: existingShoots, error: fetchError } = await db.from('shoots').select('start_time, end_time, teacher').eq('date', shootData.date).eq('studio', shootData.studio).not('start_time', 'is', null).not('end_time', 'is', null);
        if (fetchError) { console.error('Çakışma kontrolü sırasında hata:', fetchError); Swal.fire('Hata!', 'Veritabanı kontrolü sırasında bir hata oluştu.', 'error'); return; }
        let isConflict = false;
        for (const existing of existingShoots) { if (shootData.start_time < existing.end_time && shootData.end_time > existing.start_time) { isConflict = true; Swal.fire({ icon: 'error', title: 'Çakışma Var!', text: `Bu stüdyo seçtiğiniz tarih ve saat aralığında (${existing.start_time.substring(0,5)} - ${existing.end_time.substring(0,5)}) ${existing.teacher} tarafından zaten rezerve edilmiş.` }); break; } }
        if (isConflict) return;
    }
    let error;
    if (currentEditId) { ({ error } = await db.from('shoots').update(shootData).eq('id', currentEditId)); } 
    else { ({ error } = await db.from('shoots').insert([shootData])); }
    if (error) { console.error("Veri kaydetme/güncelleme hatası: ", error); Swal.fire({ icon: 'error', title: 'Hata!', text: 'Çekim planı kaydedilirken bir hata oluştu.' }); } 
    else { Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: currentEditId ? 'Çekim Planı Güncellendi' : 'Çekim Planı Eklendi', showConfirmButton: false, timer: 2000, timerProgressBar: true }); resetFormState(); }
});

// HATALI KODUN DÜZELTİLMİŞ HALİ
if (downloadPdfBtn) {
...
const shootsSubscription = db.channel('public:shoots').on('postgres_changes', { event: '*', schema: 'public', table: 'shoots' }, (payload) => { console.log('Değişiklik algılandı!', payload); fetchInitialData(); }).subscribe();
