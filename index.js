    ...
    const bodyHtml = DAYS_OF_WEEK.map(day => {
        ...
        const cellsHtml = STUDIOS.map(studio => {
            ...
            const cellContent = shootsInCell.map(shoot => {
                ...
                // KAMERAMAN GÖRÜNÜMÜ İÇİN YENİ DÜZENLEME
                const cameramen = [shoot.kameraman_1, shoot.kameraman_2].filter(Boolean); // Sadece dolu olanları al
                let cameramanDisplayHtml = cameramen.join(' / ');


                // ###############################################################
                // ##### BURASI DEĞİŞTİRİLECEK ALAN #####
                // ###############################################################
